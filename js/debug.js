...
export function getScheduleDebugInfo(state, now = new Date(), runtime = {}) {
  ...
  return {
    ...
    pdfFileName: state?.pdfImportDebug?.pdfFileName || '—',
    pdfPageCount: state?.pdfImportDebug?.pdfPageCount ?? 0,
    pdfTextItemCount: state?.pdfImportDebug?.pdfTextItemCount ?? 0,
    pdfDetectedDateSections: state?.pdfImportDebug?.pdfDetectedDateSections ?? 0,
    pdfRepairedDateSections: state?.pdfImportDebug?.pdfRepairedDateSections ?? 0,
    pdfParsedRowCount: state?.pdfImportDebug?.pdfParsedRowCount ?? 0,
    pdfSkippedRowCount: state?.pdfImportDebug?.pdfSkippedRowCount ?? 0,
    pdfInvalidRowCount: state?.pdfImportDebug?.pdfInvalidRowCount ?? 0,
    pdfTruncatedTitleCount: state?.pdfImportDebug?.pdfTruncatedTitleCount ?? 0,
    pdfParseErrors: state?.pdfImportDebug?.pdfParseErrors?.join('、') || '—',
    totalSessions: sessions.length,
    operationalDateKey: state?.operationalDateKey || '—',
    selectedOperationalDate: state?.dateFilter || 'AUTO',
    operationalSessionCount: Array.isArray(state?.visibleSessions) ? state.visibleSessions.length : 0,
    coverageReminderKey: state?.coverageReminderKey || '—',
    ...
    dcpTitleMapLoaded: state?.dcpTitleMap instanceof Map && state.dcpTitleMap.size > 0,
    dcpTitleSourceFile: state?.dcpTitleMetadata?.sourceFileName || '—',
    dcpTitleSourceSheet: state?.dcpTitleMetadata?.sourceSheetName || '—',
    dcpTitleImportedAt: formatDebugDateTime(state?.dcpTitleMetadata?.importedAt),
    dcpTitleSourceRows: state?.dcpTitleMetadata?.sourceRows ?? 0,
    dcpTitleValidRows: state?.dcpTitleMetadata?.validRows ?? 0,
    dcpTitleUniqueCount: state?.dcpTitleMap instanceof Map ? state.dcpTitleMap.size : 0,
    dcpTitleDuplicateCount: state?.dcpTitleMetadata?.duplicateRows ?? 0,
    dcpTitleConflictCount: state?.dcpTitleMetadata?.conflicts ?? 0,
    matchedSessionCount: state?.dcpTitleDebug?.matchedSessionCount ?? 0,
    unmatchedSessionCount: state?.dcpTitleDebug?.unmatchedSessionCount ?? 0,
    unmatchedSessionTitles: state?.dcpTitleDebug?.unmatchedTitles?.join('、') || '—',
