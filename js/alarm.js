...
export function createAlarmChannel() {
  ...
  async function unlock() {
    ...
    try {
      ...
      runtime.audioPlayError = '';
      return { success: true, message: '警報音效已開啟' };
    } catch (error) {
      audio.pause();
      audio.currentTime = 0;
      audio.muted = false;
  ...
  // 播放正式群組警報；呼叫端僅在 alarmEnabled 為 true 時才會呼叫此函式。
  ...
  sharedAlarmChannel = { getState, unlock, startAlarm, stopAlarm, disableAlarm, isAlarmActive };
  return sharedAlarmChannel;
}

// 提供模組外部停止正式警報的共用入口。
export function stopAlarm() {
  return createAlarmChannel().stopAlarm();
}

// 提供模組外部關閉警報的共用入口。
export function disableAlarm() {
  return createAlarmChannel().disableAlarm();
}
