# Movie Schedule Alarm V1.0 Developer Guide
...
專案必須能直接以 `index.html` 或 VS Code Live Server 執行。

V1.0 的電影資料處理完全離線，匯入後只使用 Excel 已解析的場次與英文片名。不可呼叫外部電影 metadata API、海報 API，不可儲存或讀取 metadata 快取，也不可建立 Token、外部電影資料設定或背景 metadata 載入流程。

V1.0 不顯示電影海報，不可建立海報容器、海報資產、海報載入狀態、圖片錯誤處理或海報 URL 欄位。本機資料庫功能啟用前，不可新增片名覆寫、翻譯、海報 URL 或其他外部資料欄位。
...
支援格式包含 DIG、TITAN、IMAX、ATMOS、4DX、3D、LIVE 與 SPECIAL。格式必須由片名前綴解析，搜尋、篩選與顯示使用相同的標準化值。已確認來源錯誤 `(CONAN SPECIAL J)` 必須保守修正為 `DIG SPECIAL` 規格與 `JAN` 語言；不得將其他未知前綴任意猜測為 DIG。
...
- 建立專案副本或備份專案
- 升級或更換框架
- 加入 npm、Node.js、TypeScript 或 Electron
- 還原、覆蓋或刪除使用者未提交修改
...
- V1 維持純瀏覽器、無 Node.js、完全離線的架構。
- 只有使用者未來明確要求時，才可規劃 Electron、SQLite 或其他技術。
- 若未來遷移 Electron，解析、state 與 UI 模組應維持可重用，Node API 必須透過受控 preload 層提供。
- 若未來加入 SQLite，資料庫存取必須與前端資料模型分層，UI 不得直接操作資料庫。
- 若未來需要離線中文片名或其他補充資料，應新增本機 `movies.json` 與獨立純資料 Repository 模組，另開單一功能 Commit，且不得變更 Excel Parser 的欄位偵測邏輯。
- 若未來需求與本文件衝突，以使用者最新明確需求為優先；不得自行擴張解讀。

## 二十、DCP 中文片名對照規則

- 現有 DCP 中文片名資料可由設定中心每週重複匯入。
- 新版 DCP 匯入成功後，必須以新資料完整取代舊英文／中文片名 Map，不得累加不同週資料或保留重複片名。
- 新版匯入失敗時，必須保留上一份有效 Map、metadata 與目前已套用的中文片名，不得清空或破壞既有場次。
- 重新匯入或清除 DCP Map 只可更新 `chineseTitle`、`displayTitle`、`titleMatchStatus` 與必要畫面／搜尋衍生結果，不得重建日期、倒數、Alarm 排程、場次 id 或 groupKey。
- DCP 對照必須在本機保存並於重新開啟後安全恢復；不得保存整份 XLSM、執行 VBA 巨集、呼叫網路或使用 TMDB。

## 二十一、Desktop 系統匣與場次恢復規則

- Desktop 最小化後必須隱藏至 Windows 系統匣，背景警報與週期結束提醒仍持續運作；到點時沿用 Main Process 還原及置頂流程。
- 關閉或由系統匣結束程式時，若仍有 waiting 或 playing 場次，必須先使用原生「是／否」確認；只有使用者明確確認才可終止監控。
- Main Process 只可接收關閉保護所需的狀態計數摘要，不得保存第二份完整場次資料。
- 成功匯入後只可本機保存標準化場次與必要來源 metadata，不得保存原始 Excel、PDF 或使用者營運檔案。
- 重新啟動只可將安全驗證後的保存資料恢復至唯一 `state.sessions`，並以實際時間重算狀態及後續警報；不得補響已過期場次。
- 新場次匯入成功後完整取代舊週期；匯入失敗時保留上一份有效場次。整份週期最後一場結束後只提醒一次匯入下一週資料。
